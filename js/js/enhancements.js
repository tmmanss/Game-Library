(() => {
  ...
  document.addEventListener('submit', (e) => {
    ...
    const done = (ok, msg) => {
      ...
      if (ok){ box.textContent = (msg || (window.i18n?.t?.('status.sent') || 'Отправлено! Спасибо.')); box.classList.remove('is-error'); form.reset(); }
      else { box.textContent = (msg || (window.i18n?.t?.('status.fail') || 'Не удалось отправить. Попробуйте позже.')); box.classList.add('is-error'); }
    };

    if (!isReal){ done(true, window.i18n?.t?.('status.valid') || 'Данные валидны (демо).'); return; }
    ...
    fetch(action, {
      ...
      return r.json().catch(() => ({}));
    }).then(() => done(true)).catch(err => done(false, 'Ошибка сервера: ' + (err?.message || 'неизвестно')));
  });
